// Components/VentasPorDiaTable.tsx
'use client';

import { VentasPorDiaReporte } from '@/Types/next-auth-types/VentasPorDia';
import React from 'react';

interface VentasPorDiaTableProps {
  data: VentasPorDiaReporte[];
}

export default function VentasPorDiaTable({ data }: VentasPorDiaTableProps) {
  return (
    <div>
      <h1 className="text-2xl font-semibold mb-4">Ventas Realizadas Por Dia</h1>
    <table className="min-w-full bg-white border border-gray-300 mt-4">
      ...
      <tbody>
        {data?.map((report, index) => (
          <tr key={index}>
            <td className="py-2 px-4 border-b text-center">{report.fecha}</td>
